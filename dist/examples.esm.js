...
var passwordPlugin = function passwordPlugin(options) {
  return function (event, optio) {
    return {
      transform: function transform(answer) {
        if (typeof answer === 'undefined' || answer === '') return event.defaultValue;
        return answer;
      },
      validate: function validate(answer) {
        return true;
      ...
      render: function render(err) {
        ...
        var initHandler = function initHandler(_ref) {
          var res = _ref.res;
          var result = '';

          var onInput = function onInput(data, e) {
            if (e.name === 'backspace') {
              result = result.slice(0, -1);
            } else if (e.name === 'return') {
              optio.input.removeListener('keypress', onInput);
              res(result);
            } else {
              result += data;
            }
        ...
        return new Promise(function (res) {
          optio.input.on('keypress', initHandler({
            res: res
          }));
          optio.rl.setPrompt(formatted);
          optio.rl.prompt();
...
function normalizeOptions(options) {
  ...
  options = _objectSpread$1(_objectSpread$1({}, DEFAULTS), options);
  options.input = input;
  options.output = ms;
...
var Optio = /*#__PURE__*/function () {
  ...
  _createClass(Optio, [{
    ...
    value: function createModule(events, options) {
      var optio = new Optio(events, normalizeOptions(_objectSpread$1(_objectSpread$1({}, this.options), options)));
      return optio;
    }
    /**
...
var optioApi = {
  defaultCommands: defaultCommands$1,
...
var defaultCommands = optioApi.defaultCommands;
var optio = optioApi.create(_objectSpread({}, defaultCommands));
optio.start(true);
